...
object BoolectorEngine :
    ...
    override fun getBVSort(ctx: Btor, size: Int): BoolectorSort =
        bvSortCache.getOrPut(size) { BitvecSort.bitvecSort(ctx, size) }
    ...
    override fun bvIEEE2float(ctx: Btor, expr: BoolectorNode, sort: BoolectorSort): BoolectorNode =
        bv2bv(ctx, expr, sort)

    override fun float2IEEEbv(ctx: Btor, expr: BoolectorNode, sort: BoolectorSort): BoolectorNode =
        bv2bv(ctx, expr, sort)

    override fun float2float(ctx: Btor, expr: BoolectorNode, sort: BoolectorSort): BoolectorNode =
        bv2bv(ctx, expr, sort)
    ...
    override fun makeFunction(
        ctx: Btor,
        name: String,
        retSort: BoolectorSort,
        args: List<BoolectorSort>
    ): BoolectorFunction {
    ...
    override fun binary(ctx: Btor, opcode: Opcode, lhv: BoolectorNode, rhv: BoolectorNode): BoolectorNode =
        when (opcode) {
            Opcode.EQ -> when {
    ...
    private fun concat(lhv: BitvecNode, rhv: BitvecNode): BoolectorNode =
        (lhv.toBitvecNode()).concat((rhv.toBitvecNode()))
