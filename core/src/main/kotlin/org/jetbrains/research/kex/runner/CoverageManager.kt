...
object CoverageManager {
    ...
    fun isBodyCovered(method: Method) = method.bodyBlocks.map { isCovered(it) }.fold(true) { acc, it -> acc and it }

    fun isCatchCovered(method: Method) = method.catchBlocks.map { isCovered(it) }.fold(true) { acc, it -> acc and it }
