package org.jetbrains.research.kex.state.predicate

import org.jetbrains.research.kex.state.term.Term
import org.jetbrains.research.kex.state.transformer.Transformer
import org.jetbrains.research.kex.util.unreachable
import org.jetbrains.research.kfg.type.ArrayType

class ArrayStorePredicate(arrayRef: Term, index: Term, value: Term, type: PredicateType = PredicateType.State())
    : Predicate(type, listOf(arrayRef, index, value)) {
    fun getArrayRef() = operands[0]
    fun getIndex() = operands[1]
    fun getValue() = operands[2]
    fun getComponentType() = (getArrayRef().type as? ArrayType)?.component ?: unreachable { log.error("Non-array type of array ref") }

    override fun print() = "${getArrayRef()}[${getIndex()}] = ${getValue()}"

    override fun <T: Transformer<T>> accept(t: Transformer<T>): Predicate {
        val ref = t.transform(getArrayRef())
        val indx = t.transform(getIndex())
        val store = t.transform(getValue())
        return when {
            ref == getArrayRef() && indx == getIndex() && store == getValue() -> this
            else -> t.pf.getArrayStore(ref, indx, store, type)
        }
    }
}